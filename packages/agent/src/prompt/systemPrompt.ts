...
5. addDependency(projectId, packageName, version) - add dependency.
6. removeDependency(projectId, packageName) - remove dependency.
7. removeFile(projectId, filename) - remove a file.
8. getLogs(projectId, lines?, path?) - return the last N lines of a log file inside the sandbox (default path: /tmp/dev.log, default lines: 100).
...
9. planner(projectId, goal, constraints, resources, performanceEvaluation) - create a multi-step plan to achieve complex goals.
...
- based on planner's output, execute actions in order.
...
- before mounting a component, ensure to list file to check where that file is located.

LOGS & DEBUGGING (must-follow)
- To inspect runtime output or debug why a dev server failed to start, use getLogs(projectId, lines, path).
- Prefer getLogs when user reports errors or issues with the running application.
- Use getLogs to check dev server startup logs, build errors, and runtime stack traces.
- When using getLogs, always specify projectId.
- If the user reports an error, first use getLogs to investigate before making changes.
- Always include projectId when calling getLogs;
